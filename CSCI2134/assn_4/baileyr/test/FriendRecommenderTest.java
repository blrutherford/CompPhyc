...
class FriendRecommenderTest {
  ...
  // recomendations between users with common interests
  @Test
  void makeInterestRecommendations_test() {
    User u = new User( friendNames[0] );
    User f = new User( friendNames[1] );

    u.addInterest(sportsInterest);
    f.addInterest(sportsInterest);

    FriendRecommender fr = new FriendRecommender();
    ArrayList<String> al = new ArrayList<>();
    fr.makeInterestRecommendations(u, al);
    assertEquals(1, al.size(), "Wrong recommendation count" );
    assertEquals(u.name + " and " + f.name + " should be friends", al.get( 0 ), "Incorrect recommendation");
  }

//recommending friends between no common interests shouldn't give a recommendation
  @Test
  void makeInterestRecommendations_noCommonInterests() {
    User u = new User( friendNames[0] );
    User f = new User( friendNames[1] );
    u.addInterest(sportsInterest);
    f.addInterest(tvInterest);
  ...
  @Test
  void makeInterestRecommendations_noInterest() {
    User u = new User( friendNames[0] );
    User f = new User( friendNames[1] );
    ...
    FriendRecommender fr = new FriendRecommender();
    ArrayList<String> al = new ArrayList<>();
    fr.makeInterestRecommendations(f, al);
    assertEquals( 0, al.size(), "Wrong recommendation count" );
  }
}