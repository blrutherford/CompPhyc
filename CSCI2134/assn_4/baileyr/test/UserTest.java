...
class UserTest {
  ...
  private static String sportsInterest = "Sports Hockey";
  private static String tvInterest = "TV Bridgerton";
  private static Interest sports = new Interest("Sports", "Hockey");
  private static Interest tv = new Interest("TV", "Bridgerton");
  ...
  void testAddInterest(){
    User u = new User(friendNames[0]);
    u.addInterest(sportsInterest);

    assertEquals(u.name+" starts " + sportsInterest, u.addInterest(sportsInterest), "Interest could not be added");
  }

  @Test
  void testInvalidInterest_category(){
    User u = new User(friendNames[0]);
    u.addInterest("Dance Ballet");

    assertEquals("Invalid interest", u.addInterest("Dance Ballet"), "Interest could not be added");
  }

  @Test
  void testInvalidInterest_parts(){
    User u = new User(friendNames[0]);
    u.addInterest("Sports Hockey Ball");

    assertEquals("Invalid interest", u.addInterest("Sports Hockey Ball"), "Interest could not be added");
  }

  @Test
  void testaddInterest_alreadyHasInterest(){
    User u = new User(friendNames[0]);
    u.addInterest(sportsInterest);

    assertEquals(friendNames[0] + " starts " + tvInterest, u.addInterest(tvInterest), "Interest could not be added");
  }
  ...
  void endInterest(){
    User u = new User(friendNames[0]);
    u.addInterest(sportsInterest);
    String end = u.endInterest();

    assertEquals(friendNames[0] + " ends " + sportsInterest, end, "Interest could not be removed");
  }

  @Test
  void endInterest_noInterest(){
    User u = new User(friendNames[0]);
    String end = u.endInterest();

      assertNull(end, "Interest could not be removed");
  }
  ...
  void getInterest(){
    User u = new User(friendNames[0]);
    u.addInterest(sportsInterest);
    assertEquals(sports, u.getInterest(), "Cannot access interest");
  }
