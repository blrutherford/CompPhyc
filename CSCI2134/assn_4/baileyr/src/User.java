...
public class User {
  ...
  /**
   * given an interest this method adds that interest to a user
   * @param newInterest - interest to add
   */
  public String addInterest(String newInterest) {
    //get the category and name of the interest
    String[] parts = newInterest.split(" ");
    if(parts.length != 2) {
      return "Invalid interest";
    }
    //if the interest category is invalid do not create interest
      if (interest != null) {
          endInterest();
      }
      interest = new Interest(parts[0], parts[1]);
      if(interest.category == null){
        return "Invalid interest";
      }
      return name + " starts "+ interest.category;
  }

  public String endInterest() {
    if(interest != null) {
      String n = name + " ends "+ interest.category;
      interest = null;
      return n;
    }
    return null;
