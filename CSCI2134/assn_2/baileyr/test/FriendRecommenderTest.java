...
class FriendRecommenderTest {
  ...
  /**
   *Tests that the user looking for recommendations is not recommended to themselves
   */
  @Test
  void testMakeRecommendations_notFriends() {
    User u = new User(friendNames[0]);
    User f = new User(friendNames[1]);
  ...
  /**
   * Tests if name is lexicographically larger, then it returns the right order of recommendation
   */
  @Test
  void testMakeRecommendations_nameIsLarger() {
    User u = new User(friendNames[0]);
    User f = new User(friendNames[1]);
    User g = new User(friendNames[2]);
